...
class Estimator(do_mpc.model.IteratedVariables, IndexedProperty):
...
class StateFeedback(Estimator):
    ...
    def __init__(self, model):
        # super(StateFeedback, self).__init__(model)
    ...
    **Configuration and setup:**
    ...
    def set_objective(self, stage_cost, arrival_cost):
        ...
        **Example:**
        ...
        self.stage_cost_fun = Function('stage_cost_fun', [i for i in stage_cost_input], [stage_cost])
    ...
    def set_default_objective(self, P_x, P_v=None, P_p=None, P_w=None):
        ...
        * In the case that the :py:class:`do_mpc.model.Model` is configured without measurement-noise (see :py:func:`do_mpc.model.Model.set_meas`) the parameter ``P_v`` is not required.
        ...
        if P_v is None:
            ...
            v = self._v.cat
            stage_cost += np.matmul(v.T, np.matmul(P_v, v))
        ...
        if P_w is None:
            ...
            w = self._w.cat
            stage_cost += np.matmul(w.T, np.matmul(P_w, w))
        ...
        arrival_cost = np.matmul(dx.T, np.matmul(P_x, dx))
        ...
        if P_p is None:
            ...
            dp = p_0.cat - p_prev.cat
            arrival_cost += np.matmul(dp.T, np.matmul(P_p, dp))
